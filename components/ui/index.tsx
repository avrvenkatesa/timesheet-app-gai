...
export const Modal = ({ isOpen, onClose, title, children, footer }: { isOpen: boolean; onClose: () => void; title: string; children: ReactNode; footer?: ReactNode }) => {
    ...
    return (
        <div className="printable-modal-wrapper fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={onClose}>
            <div className="printable-modal-dialog relative bg-white rounded-lg shadow-xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
                <div className="no-print flex items-start justify-between p-5 border-b rounded-t">
                ...
                {footer && (
                    <div className="no-print flex items-center justify-end p-6 space-x-2 border-t border-slate-200 rounded-b">
                        {footer}
                    </div>
                )}
            </div>
