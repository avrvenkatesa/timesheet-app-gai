...
// --- Modal ---
export const Modal = ({ isOpen, onClose, title, children, footer }: { isOpen: boolean; onClose: () => void; title: string; children: ReactNode; footer?: ReactNode }) => {
    if (!isOpen) return null;

    return (
        <div className="printable-modal-wrapper fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" onClick={onClose}>
            <div className="printable-modal-dialog relative bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="no-print flex items-start justify-between p-5 border-b rounded-t flex-shrink-0">
                ...
                {footer && (
                    <div className="no-print flex items-center justify-end p-6 space-x-2 border-t border-slate-200 rounded-b flex-shrink-0">
                        {footer}
                    </div>
                )}
            </div>
